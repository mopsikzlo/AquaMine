...
class Session{
	...
	public function update(float $time) : void{
		if(!$this->isActive and ($this->lastUpdate + 30) < $time){
		...
		if($this->state === self::STATE_DISCONNECTING and (
			...
			$this->disconnectionTime + 30 < $time)
		){
			$this->close($this->disconnectNotify);
			return;
		}
	...
	public function flagForDisconnection(bool $notify = true) : void{
		$this->state = self::STATE_DISCONNECTING;
		$this->disconnectionTime = microtime(true);
		$this->disconnectNotify = $notify;
	}
