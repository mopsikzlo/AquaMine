...
class AxisAlignedBB{
	...
	public function isVectorInside(Vector3 $vector, float $epsilon = 0.001) : bool{
		if($this->minX - $vector->x > $epsilon or $vector->x - $this->maxX > $epsilon){
			return false;
		}
		if($this->minY - $vector->y > $epsilon or $vector->y - $this->maxY > $epsilon){
			return false;
		}

		return $this->minZ - $vector->z <= $epsilon and $vector->z - $this->maxZ <= $epsilon;
	}
