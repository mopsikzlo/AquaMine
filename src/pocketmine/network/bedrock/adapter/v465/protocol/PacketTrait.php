...
namespace pocketmine\network\bedrock\adapter\v465\protocol;
...
trait PacketTrait {
    ...
    public function getItemStack(\Closure $readExtraCrapInTheMiddle) : Item{
        ...
        [$id, $meta] = ItemPalette465::getLegacyFromRuntimeId($netId, $netData);
        ...
        return (static function() use ($extraData, $netId, $id, $meta, $cnt) : Item {
            ...
            if($netId === ItemPalette465::getRuntimeFromStringId("minecraft:shield")){ //SHIELD
    ...
    public function putItemStack(Item $item, \Closure $writeExtraCrapInTheMiddle) : void{
        ...
        [$netId, $netData] = ItemPalette465::getRuntimeFromLegacyId($item->getId(),$item instanceof Durable ? 0 : $item->getDamage());
        ...
        if($isBlockItem){
            ...
            if($block->getId() !== BlockIds::AIR){
                $blockRuntimeId = BlockPalette465::getRuntimeFromLegacyId($block->getId(), $block->getDamage());
            }
        }
