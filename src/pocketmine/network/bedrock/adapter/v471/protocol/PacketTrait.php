...
trait PacketTrait{
	...
	public function getItemStack(\Closure $readExtraCrapInTheMiddle) : Item{
		...
		[$id, $meta] = ItemPalette471::getLegacyFromRuntimeId($netId, $netData);
		...
		return (static function() use ($extraData, $netId, $id, $meta, $cnt) : Item {
			...
			if($netId === ItemPalette471::getRuntimeFromStringId("minecraft:shield")){ //SHIELD
	...
	public function putItemStack(Item $item, \Closure $writeExtraCrapInTheMiddle) : void{
		...
		[$netId, $netData] = ItemPalette471::getRuntimeFromLegacyId($item->getId(),$item instanceof Durable ? 0 : $item->getDamage());
