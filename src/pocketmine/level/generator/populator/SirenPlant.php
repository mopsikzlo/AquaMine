...
class SirenPlant extends VariableAmountPopulator{
	/** @var ChunkManager */
	private $level;
	...
	public function populate(ChunkManager $level, $chunkX, $chunkZ, Random $random){
		...
		for($i = 0; $i < $amount; ++$i){
			...
			if($y !== -1 and $this->canSirenPlantStay($x, $y, $z)){
				$this->level->setBlockIdAt($x, $y, $z, 175);
				$this->level->setBlockDataAt($x, $y, $z, 1);
				$this->level->setBlockIdAt($x, $y+1, $z, 175);
				$this->level->setBlockDataAt($x, $y+1, $z, 9);				
			}
		}
	}

	private function canSirenPlantStay($x, $y, $z){
