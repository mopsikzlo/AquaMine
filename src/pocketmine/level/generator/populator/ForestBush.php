...
class ForestBush extends VariableAmountPopulator{
	/** @var ChunkManager */
	private $level;
	...
	public function populate(ChunkManager $level, $chunkX, $chunkZ, Random $random){
		...
		for($i = 0; $i < $amount; ++$i){
			...
			if($y !== -1 and $this->canForestBushStay($x, $y, $z)) {
				$rnd = mt_rand(1,3);
				if($rnd == 1){
					$this->level->setBlockIdAt($x, $y, $z, 18);
					$this->level->setBlockDataAt($x, $y, $z, 0);				
					}elseif($rnd == 2){
					$this->level->setBlockIdAt($x, $y, $z, 85);
	...
	private function canForestBushStay($x, $y, $z){
