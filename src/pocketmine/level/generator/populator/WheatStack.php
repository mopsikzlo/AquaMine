...
class WheatStack extends VariableAmountPopulator{
	/** @var ChunkManager */
	private $level;
	...
	public function populate(ChunkManager $level, $chunkX, $chunkZ, Random $random){
		...
		for($i = 0; $i < $amount; ++$i){
			...
			for($size = 35; $size > 0; $size--){
				...
				if($yy !== -1 and $this->canWheatStackStay($xx, $yy, $zz)){
				$this->level->setBlockIdAt($xx, $yy, $zz, 170);
	...
	private function canWheatStackStay($x, $y, $z){
